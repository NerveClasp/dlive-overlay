  ...
  const refresh = () => {
    fetch("http://localhost:3000/index.json")
      ...
      .then((resp) => {
        ...
        const allMessages =
          incoming.filter(({ type }) => type === "Message") || [];
        let newMessages = [];
        ...
        newMessages.forEach((message) => {
          messagesObj[message.id] = {
            ...
            content: filterXSS(message.content).replace(emojiRegex, (match) => {
              return `<img width="16px" height="16px" src="${
                emojiEndpoint + match
              }" />`;
            }),
            hide: setTimeout(() => {
...
<div class="wrapper">
  ...
  <div class="messages-wrapper">
    {#each messages as { id, content, sender }, i}
      <div class="message" transition:fade on:introstart={handleScrollIntoView}>
        <img
          class="avatar"
          src={sender.avatar}
          width="16"
        ...
        <span class="content" style={getMessageColor(sender)}>
          {@html content}
        </span>
      </div>
    {/each}
